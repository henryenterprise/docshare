...
export default function Admin() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8">
      {/* Top Header Card */}
      <div className="max-w-6xl mx-auto bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <button 
          ...
          className={`p-4 rounded-2xl border text-left transition flex flex-col justify-between ${activeTab === 'activity' ? 'bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-600/20' : 'bg-slate-900 border-slate-800 text-slate-300 hover:bg-slate-800/80'}`}
        >
          <Activity className="w-6 h-6 mb-3 opacity-80" />
          <div>
            <h3 className="font-semibold text-sm">Activity Summary</h3>
            <p className="text-xs opacity-75 mt-0.5">Real-time metrics</p>
          </div>
        </button>

        <button 
          onClick={() => setActiveTab('branding')}
          className={`p-4 rounded-2xl border text-left transition flex flex-col justify-between ${activeTab === 'branding' ? 'bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-600/20' : 'bg-slate-900 border-slate-800 text-slate-300 hover:bg-slate-800/80'}`}
        >
          <Layout className="w-6 h-6 mb-3 opacity-80" />
          <div>
            <h3 className="font-semibold text-sm">Branding & Content</h3>
            <p className="text-xs opacity-75 mt-0.5">Assets & text</p>
          </div>
        </button>

        <button 
          onClick={() => setActiveTab('canvas')}
          className={`p-4 rounded-2xl border text-left transition flex flex-col justify-between ${activeTab === 'canvas' ? 'bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-600/20' : 'bg-slate-900 border-slate-800 text-slate-300 hover:bg-slate-800/80'}`}
        >
          <Sparkles className="w-6 h-6 mb-3 opacity-80" />
          <div>
            <h3 className="font-semibold text-sm">Visual Canvas</h3>
            <p className="text-xs opacity-75 mt-0.5">Layout builder</p>
          </div>
        </button>

        <button 
          onClick={() => setActiveTab('live')}
          className={`p-4 rounded-2xl border text-left transition flex flex-col justify-between ${activeTab === 'live' ? 'bg-indigo-600 border-indigo-500 text-white shadow-lg shadow-indigo-600/20' : 'bg-slate-900 border-slate-800 text-slate-300 hover:bg-slate-800/80'}`}
        >
          <Eye className="w-6 h-6 mb-3 opacity-80" />
          <div>
            <h3 className="font-semibold text-sm">Live Display</h3>
            <p className="text-xs opacity-75 mt-0.5">Push updates</p>
          </div>
        </button>
      </div>
